...
class JackpotCelebration(FloatLayout):
    ...
    ICON_COUNT = 50
    ICON_SIZE = 220
    DRIFT_SPEED_RANGE = (80, 250)     # sideways drift speed magnitude, px/sec
...
class Slots(Widget):
    ...
    def play_result_sound(self, match_type, pumpkin_count):
        ...
        # Only cut the roll sound short for a double/triple/jackpot - a
        # generic win (e.g. a bare pumpkin bonus) or a no-match spin lets
        # the roll clip finish playing naturally.
        if snd and match_type in ('double', '3 of a kind', 'jackpot'):
            roll_snd = self.sounds.get('roll')
            if roll_snd:
                roll_snd.stop()
